...
struct rule* parse(int argc, char ** argv) {
    ...
    while (*argv) {
        ...
        if (!strcmp(*argv, "-t")) { /* Okreslenie nazwy tablicy. */
        ...
        } else if (!strcmp(*argv, "-P")) { /* ustawienie polityki */
             ...
             * (polityki ktora ma byc ustawiona)
             */
            argv++;
            if ((*argv == NULL) || (**argv == '-')) {
                parse_error("Expecting <policy> after "
                            "-P <chain>\n", &error);
                break;
            }
            ...
            /* Przyjmowane wartosci to ACCEPT lub DROP */
            ...
            /* Regula SET_POLICY poprawna: */
            r->operation = SET_POLICY;
            ...
            fprintf(stdout,
                    ...
                    "-P <chain> <ACCEPT or DROP> Set default policy for chain\n");
...
void set_policy(struct xtc_handle *h, struct rule* r) {
    ...
    if (!retval) {
        fprintf(stderr, "Chain '%s' is not built-in for '%s'!\n",
                r->chain, r->table);
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char **argv) {
    ...
    if (r->operation == DELETE_RULE) {
        ...
        create_chain(h, r);
    } else if (r->operation == SET_POLICY) { /* Zmiana domyslnej polityki */
        set_policy(h, r);
    }
